...
import com.example.neostore.model.UpdateProfileClass
...
class ResetPasswordFragment : Fragment() {
    ...
    private fun resetPassWord() {
        ...
        val accessToken = SharedPreferenceManager.getInstance(activity as MyAccountActivity).data.access_token
        ...
        RetrofitClient.getClient.changePassword(accessToken,currentPassword,newPassword,confirmedPassword).enqueue(object : Callback<UpdateProfileClass?> {
            override fun onResponse(
                ...
                if(response.body()!= null){
                    Toast.makeText((activity as MyAccountActivity).applicationContext,"${response.body()?.user_msg}",Toast.LENGTH_LONG).show()
                    
                }else{
                    Toast.makeText((activity as MyAccountActivity).applicationContext,"${response.errorBody()}",Toast.LENGTH_LONG).show()
